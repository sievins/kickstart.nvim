...
return {
  ...
  dependencies = {
    ...
    -- Useful status updates for LSP.
    {
      ...
      opts = {
        notification = {
          window = {
            winblend = 0, -- Full transparency (inherits terminal bg)
          },
        },
      },
    },
  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd('LspAttach', {
      group = vim.api.nvim_create_augroup('sievins-lsp-attach', { clear = true }),
    ...
    -- mason-lspconfig is optional now; keep it if you like :LspInstall / auto-enable behavior.
    require('mason-lspconfig').setup {
      ensure_installed = {}, -- you're using mason-tool-installer instead
      automatic_enable = false, -- we’ll enable explicitly below (optional)
    }

    -- Apply blink capabilities + your per-server settings via the new API
    ...
    -- Enable them (auto-attach based on filetype/root markers)
